...
def round_robin_alg_og(runs, qrels, pool_size, log_path):
	...
	for i in range(1, pool_size+1):
		...
		logging.debug("DOCUMENT: %s \n RANK %s", next_doc_info['DOCUMENT'], next_doc_info['RANK'])
		...
		while next_doc_info['DOCUMENT'] in retrieved_docs:
			...
			logging.debug("This document is already retrieved. Retrieving a new one:")
			logging.debug("DOCUMENT: %s \n RANK %s", next_doc_info['DOCUMENT'], next_doc_info['RANK'])

			while(next_doc.empty):
				print("run", next_run, "is empty. Trying again.")
				logging.debug("Run %s appears to be empty. Trying again.", next_run)
	...
	convert_data.write_dict_into_json(runs_status, 
																		log_path+'adjudication/round_robin_og/', 
																		"runs_status.json", 0)
